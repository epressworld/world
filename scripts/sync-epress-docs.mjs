...
async function extractRoutes(dirPath, prefix, metadataMap) {
  const files = (await fs.readdir(dirPath)).filter((file) => file.endsWith(".mjs"));
  const routes = [];

  for (const file of files) {
    const absolutePath = path.join(dirPath, file);
    const relativePath = path.relative(epressRoot, absolutePath).replaceAll("\\", "/");
...
function extractObjectKeys(content, kind) {
  ...
  while ((match = objectRegex.exec(content)) !== null) {
    ...
    const isMutationObject =
      kind === "MUTATION" && (objectName.endsWith("Mutations") || objectName.endsWith("Mutation"));
...
async function extractGraphqlOperations(graphqlRoot) {
  ...
  const queryFiles = (await fs.readdir(queriesDir)).filter((file) => file.endsWith(".mjs") && file !== "index.mjs");
  const mutationFiles = (await fs.readdir(mutationsDir)).filter((file) => file.endsWith(".mjs") && file !== "index.mjs");

  const queryMeta = {
    profile: { auth: "None", summary: "Fetch self profile metadata." },
    settings: { auth: "Optional JWT", summary: "Fetch node settings with auth-aware sensitive fields." },
    fetch: { auth: "Permission aware", summary: "Generic fetch for NODE, PUBLICATION, COMMENT models." },
    search: { auth: "Permission aware", summary: "Search nodes, publications, and comments." },
    getSiweMessage: { auth: "None", summary: "Generate SIWE challenge with nonce JWT." },
    visitor: { auth: "None", summary: "Resolve relationship between self node and visitor node." },
    nodeStatus: { auth: "None", summary: "Fetch node version and uptime start time." },
    suggestions: { auth: "None", summary: "Get mention and hashtag suggestions." },
  };

  const mutationMeta = {
    signInWithEthereum: { auth: "SIWE signature", summary: "Authenticate owner and issue JWT." },
    generateIntegrationToken: { auth: "Owner JWT", summary: "Create scoped integration token." },
    updateProfile: { auth: "Owner JWT", summary: "Update node profile metadata and assets." },
    broadcastProfileUpdate: { auth: "Owner JWT + signature", summary: "Broadcast profile update over EWP." },
    updateSettings: { auth: "Owner JWT", summary: "Update node settings transactionally." },
    subscribeNotification: { auth: "Owner JWT", summary: "Store web push subscription." },
    unsubscribeNotification: { auth: "Owner JWT", summary: "Remove web push subscription." },
    createPublication: { auth: "Owner JWT + permission", summary: "Create publication record and content." },
    updatePublication: { auth: "Owner JWT + permission", summary: "Update publication metadata." },
    destroyPublication: { auth: "Owner JWT + permission", summary: "Delete publication." },
    signPublication: { auth: "Owner JWT + signature", summary: "Sign publication and trigger replication." },
    createConnection: { auth: "Owner JWT", summary: "Follow another node and establish connection." },
    destroyConnection: { auth: "Owner JWT", summary: "Unfollow node and remove connection." },
    createComment: { auth: "Public", summary: "Create pending comment with EMAIL or ETHEREUM flow." },
    confirmComment: { auth: "Public token/signature flow", summary: "Confirm pending comment." },
    destroyComment: { auth: "Owner or commenter flow", summary: "Request comment deletion." },
    confirmCommentDeletion: { auth: "Public token flow", summary: "Confirm deletion for email-auth comment." },
  };
  ...
  for (const file of queryFiles) {
    ...
    for (const object of objects) {
      for (const key of object.keys) {
        ...
        const meta = queryMeta[key] || { auth: "Operation dependent", summary: "Generated from query module." };
        records.push({
          operationType: "QUERY",
          name: key,
  ...
  for (const file of mutationFiles) {
    ...
    for (const object of objects) {
      for (const key of object.keys) {
        if (key === "fields") continue;
        const meta = mutationMeta[key] || { auth: "Operation dependent", summary: "Generated from mutation module." };
        records.push({
  ...
  for (const segment of segments) {
    ...
    while (normalizedSegment.startsWith("//") || normalizedSegment.startsWith("/*")) {
      if (normalizedSegment.startsWith("//")) {
        ...
        normalizedSegment = normalizedSegment.slice(lineBreakIndex + 1).trimStart();
        continue;
      }
...
function formatGraphqlTypeExpression(expression, typeConstMap = {}) {
  ...
  if (trimmed.startsWith("graphql.type(")) {
    ...
    if (kind === "ObjectType" || kind === "InputObjectType" || kind === "EnumType") {
      const definition = args[1]?.trim();
      if (definition?.startsWith("{")) {
        const definitionEntries = parseTopLevelObjectEntries(definition);
        const definitionName = definitionEntries.name ? stripQuotes(definitionEntries.name) : null;
...
function mapModelFieldType(typeName) {
  ...
  if (normalized === "increments" || normalized === "integer" || normalized === "int") return "Int";
  if (normalized === "bigint" || normalized === "biginteger") return "String";
  if (normalized === "boolean" || normalized === "bool") return "Boolean";
  if (normalized === "timestamp" || normalized === "date" || normalized === "datetime") return "String";
  if (normalized === "float" || normalized === "double" || normalized === "decimal") return "Float";
...
function parseNamedTypeDefinitionFromObject(objectLiteral, kind, source) {
  ...
  if (kind === "EnumType") {
    const valuesEntries = entries.values ? parseTopLevelObjectEntries(entries.values) : {};
    return {
      name,
      kind,
      source,
  ...
  const fieldsEntries = entries.fields ? parseTopLevelObjectEntries(entries.fields) : {};
  const fieldsRaw = Object.entries(fieldsEntries).map(([fieldName, fieldDef]) => {
...
async function collectGraphqlTypeRegistry(sourceFiles, sourceContentMap) {
  ...
  for (const source of sourceFiles) {
    ...
    const constTypeRegex = /const\s+([A-Za-z0-9_]+)\s*=\s*graphql\.type\(\s*"(ObjectType|InputObjectType|EnumType)"\s*,\s*\{/g;
    let constMatch;
    while ((constMatch = constTypeRegex.exec(content)) !== null) {
      ...
      const typeDef = parseNamedTypeDefinitionFromObject(objectLiteral, kind, source);
      if (!typeDef) continue;
      typeConstMap[constName] = typeDef.name;
      mergeTypeDefinition(typeRegistry, typeDef);
    }

    const namedTypeRegex = /graphql\.type\(\s*"(ObjectType|InputObjectType|EnumType)"\s*,\s*\{/g;
    let typeMatch;
...
async function collectModelFieldRegistry() {
  ...
  for (const file of files) {
    ...
    const classMatch = content.match(/export\s+class\s+([A-Za-z0-9_]+)\s+extends\s+Model/);
    ...
    const fields = Object.entries(fieldEntries).map(([fieldName, fieldValue]) => {
...
function parseOperationArgs(argsExpression, typeConstMap, typeRegistry) {
  ...
  for (const [argName, argDefinition] of Object.entries(argsEntries)) {
    ...
    const argType = formatGraphqlTypeExpression(argMeta.type || "Unknown", typeConstMap);
    const baseType = extractBaseType(argType);
    const inputTypeDef = typeRegistry.get(baseType);
    const inputFields = inputTypeDef?.kind === "InputObjectType"
...
function buildResponseDescriptor(returnType, typeRegistry, modelRegistry) {
  const expandResponseFields = (typeName, visited = new Set()) => {
    ...
    const typeDef = typeRegistry.get(typeName);
    if (typeDef && (typeDef.kind === "ObjectType" || typeDef.kind === "InputObjectType")) {
      return (typeDef.fields || []).map((field) => {
        const nestedTypeName = extractBaseType(field.type);
  ...
  const scalarNames = new Set(["String", "Int", "Float", "Boolean", "ID", "JSON"]);
...
async function extractGraphqlOperationDetails(graphqlRoot, graphqlOperations) {
  ...
  const { typeRegistry, typeConstMap } = await collectGraphqlTypeRegistry(sourceFiles, sourceContentMap);
  ...
  const details = graphqlOperations.map((operation) => {
    ...
    const args = parseOperationArgs(operationEntries.args, typeConstMap, typeRegistry);
    const response = buildResponseDescriptor(returnType, typeRegistry, modelRegistry);
    const errorCodes = operationBlock ? collectOperationErrorCodes(operationBlock) : [];
...
async function extractEip712Types() {
  ...
  const def = {
    ...
    commentSignatureTypedData: {
      ...
      stability: "Public/App",
      scope: "Node-local",
      purpose: "Signed comment confirmation payload for Ethereum-auth comments.",
    ...
    deleteCommentTypedData: {
      ...
      stability: "Public/App",
      scope: "Node-local",
      purpose: "Signed comment deletion request payload for Ethereum-auth comments.",
      producer: "Commenter wallet",
...
function createEip712Errors() {
  return [
    ...
    },
    {
      id: "CommentSignature",
      target: "GraphQL confirmComment",
      errors: ["VALIDATION_FAILED", "NOT_FOUND", "FORBIDDEN", "EXPIRED_SIGNATURE", "INVALID_SIGNATURE"],
    },
    {
      id: "DeleteComment",
      target: "GraphQL destroyComment",
    ...
    },
    {
      id: "DATA",
      target: "POST /api/install",
      errors: ["preCheck failure", "initialSchema failure", "initialData failure"],
    },
  ];
}
...
async function main() {
  ...
  const nodeApiMetadata = {
    ...
    "POST /api/visitors": {
      ...
      errorCodes: ["400 Invalid address", "400 Invalid Ethereum address format", "500 Internal server error"],
    },
    "DELETE /api/visitors": {
      stability: "Public/App",
      auth: "None",
  ...
  const ewpEndpointsRaw = await extractRoutes(path.join(epressRoot, "server/routes/ewp"), "/ewp", ewpMetadata);
  const ewpEndpoints = ewpEndpointsRaw.map((endpoint) => {
    const item = { ...endpoint };
    delete item.source;
    return item;
  });
  const nodeRestRoutes = await extractRoutes(path.join(epressRoot, "server/routes/api"), "/api", nodeApiMetadata);
  ...
  const graphqlOperations = await extractGraphqlOperations(path.join(epressRoot, "server/graphql"));
  ...
  const protocolMapping = [
    {
      concept: "Identity",
      definition: "Node identity is rooted in Ethereum address ownership and signature verification.",
      routes: ["GET /ewp/profile", "POST /api/graphql (getSiweMessage/signInWithEthereum)"],
    },
    {
      concept: "Node",
      definition: "A node exposes profile, avatar, settings, and APIs as an autonomous social endpoint.",
      routes: ["GET /ewp/profile", "GET /ewp/avatar", "PATCH /ewp/nodes/:address"],
    },
    {
      concept: "Connection",
      definition: "Follow relationships are created and removed through signed handshakes.",
      routes: ["POST /ewp/connections", "DELETE /ewp/connections"],
    },
    {
      concept: "Publication",
      definition: "Publications bind author identity to content hash and optional signature.",
      routes: ["GET /ewp/publications", "GET /ewp/contents/:content_hash"],
    },
    {
      concept: "Replication",
      definition: "Follower nodes replicate signed content from followed publishers.",
      routes: ["POST /ewp/replications"],
    },
    {
      concept: "Proof/Signature",
      definition: "Typed-data signatures and timestamp validation enforce authenticity and replay resistance.",
  ...
  console.log(`Generated developer doc snapshots in ${path.relative(worldRoot, outputDir)}`);
}
