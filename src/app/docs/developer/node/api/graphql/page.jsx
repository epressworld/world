...
# variables
{
  ...
  "QUERY:getSiweMessage": {
    ...
    params: ["address: String! (wallet address that will sign SIWE message)"],
    response: ["JWT-backed nonce string embedded into SIWE message."],
...
# variables
{
  ...
  "MUTATION:signInWithEthereum": {
    signature:
      "signInWithEthereum(message: String!, signature: String!): String!",
    params: [
      "message: String! (SIWE message from getSiweMessage flow)",
      "signature: String! (wallet signature over SIWE message)",
    ],
    response: ["JWT token for aud=client owner session."],
    errors: [
      "INVALID_SIGNATURE",
      "EXPIRED_NONCE",
      "UNAUTHENTICATED",
  ...
  "MUTATION:generateIntegrationToken": {
    signature:
      "generateIntegrationToken(scope: [String!]!, expiresIn: String): String!",
    params: [
      "scope: [String!]! (permission list for integration token)",
      "expiresIn: String (default 24h)",
    ],
    response: ["JWT token for aud=integration with permission scope claims."],
  ...
  "MUTATION:destroyComment": {
    signature:
      "destroyComment(id: ID!, signature: String, email: String): Comment!",
    params: [
...
function buildOperationExample(detail, override = {}) {
  ...
  const hasObjectResponse =
    detail.response?.kind === "OBJECT" || detail.response?.kind === "MODEL"
  const selectionSet = hasObjectResponse
    ? buildSelectionSet(detail.response.fields || [])
    : ""
  const operationArgs = args
    .map((arg) => `${arg.name}: $${arg.name}`)
    .join(", ")
  const operationHead = args.length
    ? `${detail.name}(${operationArgs})`
    : detail.name
...
const operationDocs = graphqlOperationDetails.map((detail) => {
  ...
  const autoParams = detail.args?.length
    ...
    : []
  const params = [...autoParams, ...(override.params || [])]
  if (params.length === 0) {
  ...
  if (detail.response?.kind === "OBJECT" || detail.response?.kind === "MODEL") {
    ...
    const values = (detail.response.values || []).map(
      (value) => `enum: ${value}`,
    )
  ...
  return {
    ...
    auth: detail.auth,
    params,
    body: ["GraphQL request body: { query, variables }", ...params],
    response,
    errors:
      override.errors ||
      (detail.errorCodes?.length
        ? detail.errorCodes
    ...
    <div className="space-y-6">
      <article className="rounded-2xl border border-dark-border bg-dark-surface/70 p-6 md:p-8">
        ...
        <p className="mt-3 leading-7 text-gray-300">
          GraphQL is served under{" "}
          <span className="font-mono">/api/graphql</span>. This chapter
          documents all current queries and mutations with arguments, response
          schema, error codes, and implementation links.
        </p>
