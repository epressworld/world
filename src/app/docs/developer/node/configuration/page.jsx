  ...
  return (
    <div className="space-y-6">
      <article className="rounded-2xl border border-dark-border bg-dark-surface/70 p-6 md:p-8">
        <h1 className="text-3xl font-bold text-white">epress node Configuration</h1>
      ...
      <article className="rounded-xl border border-dark-border bg-dark-surface/70 p-6">
        <h2 className="text-xl font-semibold text-white">Infrastructure Variables (.env as Source of Truth)</h2>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody className="text-gray-300">
              {infraConfig.map((item) => (
                <tr key={item[0]} className="border-b border-dark-border/60 last:border-b-0">
                  <td className="px-2 py-2 font-mono text-primary">{item[0]}</td>
      ...
      <article className="rounded-xl border border-dark-border bg-dark-surface/70 p-6">
        <h2 className="text-xl font-semibold text-white">Recommended Override Strategy</h2>
        <p className="mt-2 text-gray-300">
          Keep repository <span className="font-mono">.env</span> unchanged and place environment-specific overrides in
          <span className="font-mono"> .env.local</span>. This aligns with the project workflow for pulling upstream updates
          without local config conflicts.
        </p>
      ...
      <article className="rounded-xl border border-dark-border bg-dark-surface/70 p-6">
        <h2 className="text-xl font-semibold text-white">Application-Level Settings</h2>
        <p className="mt-2 text-gray-300">
          Runtime product settings are persisted in the settings table and include follow/comment toggles, theme/language,
          RSS, mail transport, and push notification keys.
        </p>
        <ul className="mt-3 list-disc space-y-2 pl-6 text-gray-300">
          <li>Settings are initialized during /api/install seed stage.</li>
          <li>GraphQL settings query/mutations expose and update these values.</li>
          <li>Sensitive fields are conditionally exposed depending on auth context.</li>
        </ul>
      </article>
    </div>
  );
