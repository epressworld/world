...
const followFlow = {
  ...
  nodes: [
    { label: "Follower Node" },
    { label: "Followee Node" },
    { label: "Connection Store" },
  ],
  exchanges: [
    {
      from: 0,
      to: 1,
      label: "POST /ewp/connections + typedData",
      tone: "amber",
    },
  ...
  exchanges: [
    {
      from: 0,
      to: 1,
      label: "POST /ewp/replications + statementOfSource",
      tone: "amber",
    },
    { from: 1, to: 1, label: "verify signature + follow state", tone: "blue" },
    {
      from: 1,
      to: 2,
      label: "fetch content + commit publication",
      tone: "amber",
    },
  ],
  footer:
    "Content enters timeline only when proof and relationship are both valid",
}
  ...
  exchanges: [
    {
      from: 0,
      to: 1,
      label: "PATCH /ewp/nodes/:address + typedData",
      tone: "amber",
    },
    { from: 1, to: 1, label: "address match + signer recovery", tone: "blue" },
    {
      from: 1,
      to: 2,
      label: "apply only if timestamp is newer",
      tone: "amber",
    },
  ],
    ...
    <div className="space-y-6">
      <article className="rounded-2xl border border-dark-border bg-dark-surface/70 p-6 md:p-8">
        ...
        <p className="mt-3 leading-7 text-gray-300">
          In <span className="text-white">epress world protocol</span>,
          signatures are not only identity proofs. They are state-transition
          permissions for node-to-node actions. This model defines where trust
          is accepted, where payloads are rejected, and how replay and tampering
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <div className="rounded-lg border border-dark-border bg-dark-bg/60 p-4">
            ...
            <p className="mt-2 text-sm text-gray-300">
              Recovered signer address must match protocol identity fields
              inside the message.
            </p>
          </div>
          <div className="rounded-lg border border-dark-border bg-dark-bg/60 p-4">
            ...
            <p className="mt-2 text-sm text-gray-300">
              Timestamp windows reject expired payloads and stale update
              attempts.
            </p>
          </div>
          <div className="rounded-lg border border-dark-border bg-dark-bg/60 p-4">
            ...
            <p className="mt-2 text-sm text-gray-300">
              Even valid signatures cannot bypass relationship checks, resource
              checks, or ownership checks.
            </p>
          </div>
        </div>
      </article>
