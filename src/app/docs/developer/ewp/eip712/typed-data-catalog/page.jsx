    ...
    <div className="space-y-6">
      <article className="rounded-2xl border border-dark-border bg-dark-surface/70 p-6 md:p-8">
        ...
        <p className="mt-3 leading-7 text-gray-300">
          Field-level catalog for every implemented EIP-712 <span className="font-mono">primaryType</span> currently used by protocol or node workflows.
        </p>
      </article>

      <section className="space-y-4">
        <h2 className="text-xl font-semibold text-white">Cross-node Protocol Types</h2>
        {crossNode.map((item) => (
      ...
      <section className="space-y-4">
        <h2 className="text-xl font-semibold text-white">Node-local Runtime Types</h2>
        {nodeLocal.map((item) => (
