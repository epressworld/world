    ...
    <div className="space-y-6">
      <article className="rounded-2xl border border-dark-border bg-dark-surface/70 p-6 md:p-8">
        ...
        <p className="mt-3 leading-7 text-gray-300">
          EIP-712 typed data and Ethereum signatures are the trust backbone of{" "}
          <span className="text-white">epress world protocol</span>. For
          state-changing or provenance-critical interactions, nodes depend on
          typed-data signatures to verify signer identity, payload integrity,
          and intent timestamp.
        </p>
      </article>
      ...
      <article className="rounded-xl border border-primary/35 bg-primary/10 p-6">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-6 text-gray-200">
          <li>
            Read-only discovery endpoints such as{" "}
            <span className="font-mono">GET /ewp/profile</span> and{" "}
            <span className="font-mono">GET /ewp/publications</span> are not
            signed requests.
          </li>
          <li>
            Cross-node state transitions and content provenance rely on EIP-712
            signatures.
          </li>
          <li>
            Verification includes signer recovery, identity binding, and
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <article className="rounded-xl border border-dark-border bg-dark-surface/70 p-5">
          ...
          <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-gray-300">
            {crossNode.map((item) => (
              <li key={`cross-${item.id}`} className="font-mono">
                {item.primaryType}
              </li>
            ))}
          </ul>
        ...
        <article className="rounded-xl border border-dark-border bg-dark-surface/70 p-5">
          ...
          <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-gray-300">
            {nodeLocal.map((item) => (
              <li key={`local-${item.id}`} className="font-mono">
                {item.primaryType}
              </li>
            ))}
          </ul>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        >
          ...
          <p className="mt-2 text-sm text-dark-muted">
            Structured field-level definitions for every current primaryType.
          </p>
        </Link>
        <Link
        ...
        >
          ...
          <p className="mt-2 text-sm text-dark-muted">
            Server-side acceptance checks and error mapping per type.
          </p>
        </Link>
        <Link
        ...
        >
          ...
          <p className="mt-2 text-sm text-dark-muted">
            Replay resistance, signer binding, and trust boundaries with flow
            diagrams.
          </p>
        </Link>
      </div>
