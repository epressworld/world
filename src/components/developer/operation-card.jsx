...
export function OperationCard({
  ...
  return (
    <article id={id} className="scroll-mt-24 rounded-xl border border-dark-border bg-dark-surface/70 p-5">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {path ? <span className="font-mono text-xs text-gray-200">{path}</span> : null}
        {stability ? (
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <div className="rounded-lg border border-dark-border bg-dark-bg/50 p-4">
          <p className="text-xs uppercase tracking-wide text-dark-muted">Auth</p>
          <p className="mt-1 text-sm text-gray-200">{auth}</p>
          <p className="mt-3 text-xs uppercase tracking-wide text-dark-muted">Params</p>
          <DetailList items={params} />
        </div>
        <div className="rounded-lg border border-dark-border bg-dark-bg/50 p-4">
          <p className="text-xs uppercase tracking-wide text-dark-muted">Body</p>
          <DetailList items={body} />
          <p className="mt-3 text-xs uppercase tracking-wide text-dark-muted">Response</p>
      ...
      {errors && errors.length > 0 ? (
        <div className="mt-4 rounded-lg border border-dark-border bg-dark-bg/50 p-4">
          <p className="text-xs uppercase tracking-wide text-dark-muted">Error Codes</p>
          <ul className="mt-2 grid gap-1 text-sm text-gray-300 md:grid-cols-2">
            {errors.map((item) => (
              <li key={item}>
                <span className="font-mono text-primary">{item.split(" ")[0]}</span>
      ...
      {example ? (
        <div className="mt-4">
          <p className="text-xs uppercase tracking-wide text-dark-muted">Example</p>
